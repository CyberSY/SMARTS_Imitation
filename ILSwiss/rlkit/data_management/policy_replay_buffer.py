...
class PolicyReplayBuffer:
    def __init__(self, max_replay_buffer_size, env, random_seed=1995):
        ...
        self.default_agent_name = env.default_agent_name
        self.default_agent_buffer = AgentEnvReplayBuffer(
            max_replay_buffer_size,
            self._observation_space_n[self.default_agent_name],
            self._action_space_n[self.default_agent_name],
        )
        # TODO(zbzhu): MAYBE change agent_buffers to policy_buffers
        self.agent_buffer_n = {
            a_id: self.default_agent_buffer
    ...
    def add_path(self, path_n, absorbing=False, env=None):
        self.default_agent_buffer.add_path(
            path_n[self.default_agent_name],
            absorbing=absorbing,
            env=env
        )
        # for a_id in self.agent_ids:
